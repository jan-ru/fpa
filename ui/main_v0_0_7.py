...
from utils.dbt_utils import get_dbt_run_status
...
with ui.tab_panels(main_tabs, value=dashboard_tab).classes('w-full').style('padding-bottom: 50px;'):
    ...
    with ui.tab_panel(inputs_tab):
        ...
        create_system_status_card()
        
        # Lazy-loaded Excel files data
        file_columns = ['filename', 'size_mb', 'n_columns', 'n_rows', 'total_debit', 'total_credit', 'modified', 'status']
        create_lazy_data_card(
            title='Excel Files in Data Directory',
            subtitle='File inventory and processing status',
    ...
    with ui.tab_panel(dashboard_tab):
        ...
        with ui.element('div').classes('box mb-4'):
            ...
            with ui.element('div').classes('field is-grouped mt-2'):
                ui.button('Apply Filters', on_click=lambda: update_grids_and_stats()).classes(Styles.EMPHASIZED_BUTTON_CLASSES).style(get_button_style('compact'))
                ui.button('Clear All', on_click=lambda: filter_manager.clear_all_filters()).classes(Styles.TRANSPARENT_BUTTON_CLASSES + ' ml-2').style(get_button_style('compact'))
        ...
        with ui.tab_panels(data_tabs, value=accounts_data_tab).classes('w-full'):
            # Accounts grid with lazy loading
            with ui.tab_panel(accounts_data_tab):
                ...
                create_lazy_data_card(
                    ...
                    subtitle='Overview of all accounts with balances and activity',
                    data_key='accounts',
                    table_columns=account_columns,
            ...
            # Transactions grid with lazy loading
            with ui.tab_panel(transactions_data_tab):
                ...
                create_lazy_data_card(
                    ...
                    subtitle='Detailed view of financial transactions',
                    data_key='transactions',
                    table_columns=transaction_columns,
    ...
    with ui.tab_panel(admin_tab):
        ...
        iceberg_status = [
            {'icon': Icons.SUCCESS, 'text': 'Iceberg tables active'},
            {'icon': Icons.SUCCESS, 'text': 'Snapshots available'},
            {'icon': Icons.INFO, 'text': 'Data versions: 4 snapshots'},
            {'icon': Icons.INFO, 'text': 'Latest snapshot: 2 hours ago'}
        ]
        ...
        # Get real dbt status
        dbt_run_info = get_dbt_run_status()
        ...
        dbt_status = [
            ...
            {'icon': dbt_run_info['icon'], 'text': dbt_run_info['last_run']}
        ]
        
        dbt_buttons = [
            {'text': 'Run dbt', 'on_click': lambda: ui.notify(StatusMessages.FEATURE_COMING_SOON), 'type': 'emphasized'},
            {'text': 'Test dbt', 'on_click': lambda: ui.notify(StatusMessages.FEATURE_COMING_SOON), 'type': 'transparent'}
        ]
        ...
        cache_buttons = [
            {'text': 'Clear Cache', 'on_click': lambda: smart_cache.invalidate(), 'type': 'transparent'},
            {'text': 'Cache Stats', 'on_click': lambda: ui.notify(f'Cache statistics: {cache_stats}'), 'type': 'transparent'}
        ]
